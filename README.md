...
还有一类更隐蔽的错位，症状是**每一部分的第一章**出问题
（`test/fixtures/part-e*.epub` 钉住四种写法）。

「第一部分」「Part I」这类条目常常不拥有自己的文件，只是个壳。常见三种写法：

```xml
<li><a href="part1.xhtml">第一部分</a>   ← 有独立扉页，没问题
<li><a href="ch1.xhtml">第一部分</a>     ← href 直接指向它第一章的文件
<li><span>第一部分</span>                ← 干脆没有链接
```

第二种最坑：TOC 索引是先到先得的，壳先占住了 `ch1.xhtml`，
第一章再来就没位置了 —— 没锚点就被**直接丢弃**，有锚点就降级成锚点条目
**排到本部分的末尾**。每部分只有第一章会撞上壳，所以错位精确地落在第一章。
第三种则是整个部分的标题消失、章节全平铺。

修法是把「分组」和「文件条目」分开：`buildTocIndex()` 除了 `index`
还返回 `groups`，两条判定规则是

- **壳让位**：一个条目的**第一个直接子节点**跟它同文件时，它降级为分组，
  只保留标题，把 href 让给子条目。只看第一个子，是为了不误伤
  「章 + 章内小节」—— 那里指回自己的锚点子条目是真实章节的一部分，父不能降级
- **无名节点不建分组**：解析不出标题的占位 `<li>` 直接透传父级，
  否则跳级目录里会多出一层空文件夹

分组没有自己的文件，也就没有 spine 位置，所以建树时**按需创建**，
位置由第一个挂进来的子决定 —— 遍历本来就是 spine 顺序，分组自然落在它第一章那里。
